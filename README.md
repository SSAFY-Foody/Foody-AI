...
### 개선사항
### App_v2
RDB 와 Chroma DB 에 들어있는 값은 모두 다 값 </br>
-> DB 관리가 굉장히 효율적이지 못함 </br>
해당 문제점을 다음과 같이 해결함 </br>
1. Chroma DB 의 값은 Food 코드, 이름 값만 저장
2. 키워드 추출후 RAG 탐색은 Chroma 에서 진행이 이루어지고 영양소 정보는 RDB에서 join 하는 형식으로 값을 가져오도록 유도
</br>
